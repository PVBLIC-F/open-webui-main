...
export type SpendLimitsResponse = SpendSummary & {
	spend_limit_enabled: boolean;
	spend_limit_daily: number | null;
	spend_limit_monthly: number | null;
};

export const getUserSpendLimits = async (token: string, userId: string): Promise<SpendLimitsResponse | null> => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/users/${userId}/spend-limits`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
...
export const updateUserSpendLimits = async (
	token: string,
	userId: string,
	limits: SpendLimitsForm
) => {
	let error = null;
...
export const getMySpend = async (token: string): Promise<SpendSummary | null> => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/users/me/spend`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
...
export const getMySpendLimits = async (token: string): Promise<SpendLimitsResponse | null> => {
	let error = null;

	const res = await fetch(`${WEBUI_API_BASE_URL}/users/me/spend-limits`, {
		method: 'GET',
		headers: {
			'Content-Type': 'application/json',
			Authorization: `Bearer ${token}`
