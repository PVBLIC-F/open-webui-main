...
class GmailSyncStatus(Base):
    ...
    sync_status = Column(String, default="never")            # "never", "active", "paused", "error"
...
class GmailSyncStatusTable:
    ...
    def mark_sync_complete(
        ...
        return self.update_sync_status(
            ...
            total_emails_synced=GmailSyncStatus.total_emails_synced + emails_synced
        )
    ...
    def mark_sync_error(self, user_id: str, error_message: str) -> Optional[GmailSyncStatusModel]:
        ...
        return self.update_sync_status(
            ...
            error_count=GmailSyncStatus.error_count + 1
        )
    ...
    def get_users_needing_sync(self, max_hours_since_sync: int = 24) -> list[str]:
        ...
        try:
            with get_db() as db:
                ...
                users = db.query(GmailSyncStatus).filter(
                    GmailSyncStatus.sync_enabled == True,
                    GmailSyncStatus.auto_sync_enabled == True,
                    GmailSyncStatus.sync_status != "active",  # Don't sync if already running
                    (
                        (GmailSyncStatus.last_sync_timestamp == None) |  # Never synced
                        (GmailSyncStatus.last_sync_timestamp < cutoff_time)  # Stale sync
                    )
                ).all()
