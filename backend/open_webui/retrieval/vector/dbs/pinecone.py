...
class PineconeClient(VectorDBBase):
    ...
    def upsert(self, collection_name: str, items: List[VectorItem], namespace: str = None) -> None:
        ...
        log.info(f"Upserting {len(items)} items to Pinecone collection: {collection_name} (with prefix: {collection_name_with_prefix}){namespace_info}")
        if items and items[0].get("metadata"):
            sample_metadata = items[0]["metadata"]
            log.info(f"Sample metadata - file_id: {sample_metadata.get('file_id')}, name: {sample_metadata.get('name')}, user_id: {sample_metadata.get('user_id')}")
        ...
        for i in range(0, len(points), BATCH_SIZE):
            ...
            if namespace:
                futures.append(executor.submit(self.index.upsert, vectors=batch, namespace=namespace))
            else:
                futures.append(executor.submit(self.index.upsert, vectors=batch))
        for future in concurrent.futures.as_completed(futures):
            try:
    ...
    def query(
        self, collection_name: str, filter: Dict, limit: Optional[int] = None, namespace: str = None
    ) -> Optional[GetResult]:
